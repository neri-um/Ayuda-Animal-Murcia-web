...
public class AlmacenController {
    ...
    public ResponseEntity<SolicitudProducto> crearSolicitud(
            @RequestBody SolicitudNuevaDTO dto) {
        return ResponseEntity.ok(servicio.crearSolicitud(
                dto.getVoluntarioId(), dto.getProductoId(),
                dto.getCantidad(), dto.getMotivo()));
    }

    @PutMapping("/solicitudes/{id}")
    @PreAuthorize("hasAnyAuthority('ADMIN', 'ENCARGADO')")
    public ResponseEntity<SolicitudProducto> decidirSolicitud(
            @PathVariable Long id,
            @RequestBody DecisionSolicitudDTO dto) {
        return ResponseEntity.ok(servicio.decidirSolicitud(
                id, dto.getDecision(), dto.getEncargadoId()));
    }
    ...
    @PreAuthorize("hasAnyAuthority('ADMIN', 'ENCARGADO')")
    public ResponseEntity<AsignacionProducto> confirmarDevolucion(
            @PathVariable Long id,

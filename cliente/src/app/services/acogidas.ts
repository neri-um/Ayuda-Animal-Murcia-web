...
export async function crearAcogedor(datos: AcogidaRequest): Promise<Acogida> {
...
export async function listarAcogedores(token: string): Promise<Acogida[]> {
  const res = await fetch(`${BASE}/acogidas`, {
...
export async function cambiarEstadoAcogedor(token: string, id: number, estado: EstadoAcogida): Promise<Acogida> {
...
export async function eliminarAcogedor(token: string, id: number): Promise<void> {

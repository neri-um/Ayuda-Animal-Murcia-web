    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <div className="flex flex-col gap-5">
          ...
          {animal.status === 'EN_ADOPCION' && <div className="mt-2 flex flex-col gap-2"><div className="flex flex-row gap-2"><Link to={`/adopcion/${toSlug(animal.name)}`} className="flex items-center justify-center gap-1.5 w-full py-2.5 px-2 rounded-xl text-sm transition-all hover:opacity-80 whitespace-nowrap" style={{ backgroundColor: '#f7e3b0', color: '#2e2e2e', fontWeight: 600 }}><Heart className="w-4 h-4 flex-shrink-0" />Quiero adoptar a {animal.name}</Link>{animal.needsAcogida && <Link to={`/acogida/${toSlug(animal.name)}`} className="flex items-center justify-center gap-1.5 w-full py-2.5 px-2 rounded-xl text-sm transition-all hover:opacity-80 border whitespace-nowrap" style={{ backgroundColor: '#d9d9d9', color: '#2e2e2e', fontWeight: 600, borderColor: '#d9d9d9' }}><Home className="w-4 h-4 flex-shrink-0" />Quiero acoger a {animal.name}</Link>}</div><p className="text-center text-xs text-gray-400 mt-2">Te contactaremos lo antes posible</p></div>}

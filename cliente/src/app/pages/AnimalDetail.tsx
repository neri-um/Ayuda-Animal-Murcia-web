    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <div className="flex flex-col gap-5">
          ...
          {traits.length > 0 && <div><h3 className="text-gray-800 mb-3">Características</h3><div className="flex flex-wrap gap-2">{traits.map((t, i) => <div key={i} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm border" style={{ backgroundColor: '#f0e8d0', color: '#2e2e2e', borderColor: '#d9d0b8' }}><CheckCircle className="w-3.5 h-3.5" style={{ color: '#e8a020' }} />{t.label}</div>)}</div></div>}
          ...
          {animal.status === 'EN_ADOPCION' && <div className="mt-2"><Link to={`/adopcion/${toSlug(animal.name)}`} className="flex items-center justify-center gap-2 w-full py-3.5 rounded-xl transition-all hover:opacity-80" style={{ backgroundColor: '#f7e3b0', color: '#2e2e2e', fontWeight: 600 }}><Heart className="w-5 h-5" />Quiero adoptar a {animal.name}</Link><p className="text-center text-xs text-gray-400 mt-2">Te contactaremos en menos de 48h</p></div>}

...
import {
  listarAcogedores, cambiarEstadoAcogedor, eliminarAcogedor,
...
export default function AcogidaManagement() {
  const { token } = useAuth();
  const [tab, setTab] = useState<'acogedores' | 'solicitudes'>('acogedores');
  const [acogedores, setAcogedores] = useState<Acogida[]>([]);
  ...
  const cambiarEstado = async (id: number, estado: EstadoAcogida) => {
    if (!token) return;
    try {
      await cambiarEstadoAcogedor(token, id, estado);
      await cargar();
    } catch (e: any) {
  ...
  const borrarAcogedor = async (id: number) => {
    if (!token) return;
    if (!window.confirm('¿Eliminar este acogedor y su solicitud?')) return;
    try {
      await eliminarAcogedor(token, id);
      await cargar();
    } catch (e: any) {
  ...
  const nombresEspecie = (a: Acogida): string => (a.especie ? a.especie : 'PERRO');

  const acogedoresFiltrados = acogedores.filter(a => {
  ...
  const contadoresEspecie = {
    TODAS: acogedores.length,
    PERRO: acogedores.filter(a => a.especie === 'PERRO').length,
    GATO: acogedores.filter(a => a.especie === 'GATO').length,
  };

  const contadoresEstado: Record<string, number> = {
    TODOS: acogedores.length,
    ...ESTADOS_ACOGIDA.reduce((acc, e) => ({ ...acc, [e]: acogedores.filter(a => a.estado === e).length }), {}),
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Acogedores disponibles y solicitudes pendientes</p>
        </div>
      ...
      <div className="flex gap-2">
        <button
          onClick={() => setTab('acogedores')}
          className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors border ${
            tab === 'acogedores' ? 'text-white border-transparent' : 'bg-white text-gray-500 border-gray-200'
          }`}
          style={tab === 'acogedores' ? { backgroundColor: '#547792' } : {}}
        >
          <span className="inline-flex items-center gap-1.5"><Users className="w-4 h-4" /> Acogedores</span>
        </button>
        <button
        ...
        <>
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <input
                type="text"
                placeholder="Buscar acogedor por nombre..."
                value={busqueda}
          ...
          {acogedoresFiltrados.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <Home className="w-10 h-10 text-gray-300 mb-3" />
              <p className="text-gray-500 text-sm">No hay acogedores con esos filtros.</p>
            </div>
          ) : (
            <div className="space-y-3">
              {acogedoresFiltrados.map(a => (

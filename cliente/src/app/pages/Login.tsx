...
export default function Login() {
  usePageMeta({
    title: 'Acceso | Ayuda Animal Murcia',
    description: 'Acceso al panel de administración de Ayuda Animal Murcia.',
    path: '/login',
    noindex: true,
  });
  ...
  const [avisoCaducada, setAvisoCaducada] = useState(() => {
    try { return sessionStorage.getItem('va_sesion_caducada') === '1'; } catch { return false; }
  });
  const { login } = useAuth();
        ...
        <div style={{
          ...
          {avisoCaducada && (
            <div style={{
              ...
              color: '#92400e', fontSize: '0.82rem', marginBottom: '0.875rem',
            }}>
              <Clock className="w-4 h-4 flex-shrink-0" style={{ marginTop: '0.05rem' }} />
              <span>Tu sesión ha caducado. Inicia sesión de nuevo para continuar.</span>
            </div>
          )}

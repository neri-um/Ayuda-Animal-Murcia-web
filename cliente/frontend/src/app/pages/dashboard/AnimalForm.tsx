...
export default function AnimalForm() {
  ...
  const { animals, token, fetchAnimals } = useApp();
    ...
    if (isEdit && id && !animalLoadedRef.current) {
      ...
      if (animal) {
        ...
        if (!puedeEditar) {
          ...
          navigate('/dashboard/animals', { replace: true });
          return;
        }
  ...
  }, [id, isEdit, animals, esAdmin, currentUser?.id, navigate]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const body = {
      ...
      vacunado: form.vaccinated,
      esterilizado: form.sterilized,
    ...
    try {
      ...
      if (!res.ok) {
        alert('Error al guardar. Código: ' + res.status);
        setSaving(false);
        return;
      }
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <section className="bg-white rounded-2xl border border-gray-100 p-6">
          ...
          <div className="mt-4">
            ...
            <textarea value={form.description} onChange={e => update('description', e.target.value)}
              rows={4} placeholder="Describe la personalidad, historia y necesidades del animal..."
        ...
        <section className="bg-white rounded-2xl border border-gray-100 p-6">
          <h3 className="text-gray-800 mb-5 pb-3 border-b border-gray-100">Salud y características</h3>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {([
              { key: 'vaccinated', label: 'Vacunado/a' },
              { key: 'sterilized', label: 'Esterilizado/a' },
